...
sys.path.append('/home/nasmonir/PhD/se-up-evaluations/models/tango/tango/')
...
def get_parsed_arg():
    ...
    model_dir = '/home/nasmonir/PhD/se-up-evaluations/models/tango/tango/tests/test_rsc/dnn'
    parser.add_argument(
        "-m",
        "--model-paths",
        type=str,
...
def main(model_paths, mu, rank, steps, mic_ref):
...
if __name__ == "__main__":
    ...
    for scenario in ['0dBS0N45', '0dBS0N90', 'p5dBS0N45', 'p5dBS0N90', 'm5dBS0N45', 'm5dBS0N90']:
        
        # Get paths
        print(f"[INFO] SCENARIO : {scenario}")
        print('-'*150)
        mixtures_paths = f'/srv/storage/talc3@storage4.nancy.grid5000.fr/multispeech/calcul/users/nmonir/ansd-se-data/nmansd/simulations/with-speech-shaped-noise/{scenario}/with-kemar/*/mixture.wav' # TMP
        estimated_speeches_dir = f'/srv/storage/talc3@storage4.nancy.grid5000.fr/multispeech/calcul/users/nmonir/ansd-se-data/nmansd/enhancements/{scenario}/tango/with-kemar/{args.noise_type}/'        
        # Load the model
        tango_filter, sn_model, mn_model = main(model_paths=args.model_paths, mu=args.mu, rank=args.rank, steps=args.steps, mic_ref=args.ref_mic)

        # Multi-inference with respect to the scenario 
        for mixture_path in tqdm(glob.glob(mixtures_paths)):
            filename = mixture_path.split('/')[-2]
            output_dir = estimated_speeches_dir + filename
            ...
            # Paths 
            estimated_speech_path = output_dir + '/' + 'output.wav'
            estimated_speech_ch0_path = estimated_speech_path.replace('output.wav', 'estimated_speech_ch0.wav')
            estimated_speech_ch2_path = estimated_speech_path.replace('output.wav', 'estimated_speech_ch2.wav')

            # Load output.wav 
            estimated_speech, sr = librosa.load(estimated_speech_path, sr=None, mono=False)
            sf.write(estimated_speech_ch0_path, estimated_speech[0], sr)
            sf.write(estimated_speech_ch2_path, estimated_speech[1], sr)
